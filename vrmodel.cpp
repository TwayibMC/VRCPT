...
void VRModel::processNode(aiNode* node, const aiScene* scene)
{
    ...
    for(GLuint i = 0; i < node->mNumChildren; i++)
    {
        ...
        //qDebug() << numMaterial << "appel recursif";
        numMaterial++;
    }
}
...
void VRModel::setOpacity(float newOpacity)
{
    VRBody::setOpacity(newOpacity);
    foreach (VRMesh* mesh, meshes) {
        mesh->setOpacity(newOpacity);
    }
}

void VRModel::setSpecStrength(float spec) {
    VRBody::setSpecStrength(spec);
    foreach (VRMesh* mesh, meshes) {
        mesh->setSpecStrength(spec);
    }
}
